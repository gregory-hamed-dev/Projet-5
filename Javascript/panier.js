...
const renderCart = (imgUrl, name, color, price, quantity) => {
    ...
    <tr class="text-center">
        <th class="head" scope="row"><img src="${imgUrl}"></th>
        <td>${name}</td>
        <td>${color}</td>
        <td>${price}€</td>
        <td><i class="fas fa-minus"></i>${quantity}<i class="fas fa-plus"></i></td>
...
const regexEmail = new RegExp("^[a-zA-Z0-9.-_]+[@]{1}[a-z]+[.]{1}[a-z]{2,3}$", "g"); // expression régulière pour valider plusieurs types d'adresse mail
...
const isValidEmail = (value) => (regexEmail.test(value) ? true : false); // test pour vérifier que l'utilisateur a bien rentré un format correct d'adresse mail
